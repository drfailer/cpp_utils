...
#define uassert_equal(lhs, rhs)                                                \
  utest::Test::assert_equal("ASSERT", lhs, rhs, #lhs, #rhs, __FILE__, __LINE__)
#define urequire(expr)                                                         \
  if (utest::Test::assert_true("REQUIRE", expr, #expr, __FILE__, __LINE__) !=  \
      0) {                                                                     \
  ...
  static inline void report(std::string const &test_name) {
    if (!require_failed && nb_assert_failed == 0) {
      ...
      if (require_failed) {
        ...
        std::cerr << nb_assert_failed << " assertion failed." << std::endl;
      }
    }
  }
